$(document).ready(function() {
    ...
    // Размеры буфера (меньше = больше зерно)
    var bufferWidth, bufferHeight;
    
    function setup() {
        // Размер буфера зависит от размера зерна
    ...
    function generateNoise() {
        // Создаем уменьшенный буфер для больших зерен
        ...
        // Масштабируем буфер на весь canvas
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        ctx.imageSmoothingEnabled = false; // Отключаем сглаживание для четких зерен
        ctx.drawImage(buffer, 0, 0, bufferWidth, bufferHeight, 
                      0, 0, canvas.width, canvas.height);
    }
    
    // Анимация с регулируемой скоростью
    var noiseInterval;
    function startNoise() {
    ...
    // Опционально: динамическое изменение параметров
    ...
    // updateSettings({grainSize: 10, updateInterval: 200});
});